...
from app.dependencies import get_classifier_agent, get_db_session, get_reconciliation_agent
...
async def client(session_factory):
    ...
    def stub_recon_agent():
        class _A:
            async def run(self, p):
                class _R:
                    output = ReconciliationAnalysis(
                        accounts=[
                            AccountAnalysis(
                                account_code=100101,
    ...
    app.dependency_overrides[get_reconciliation_agent] = stub_recon_agent
