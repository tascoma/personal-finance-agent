...
Real LLM calls are stubbed: the agent dependencies are overridden with
``_StubAgent`` instances that return fixed extraction results. CSV/XLSX paths
...
def _empty_statement_agent() -> _StubAgent:
    return _StubAgent(ExtractedStatement(transactions=[]))


def _empty_paystub_agent() -> _StubAgent:
    return _StubAgent(
        ExtractedPaystubs(
            paystubs=[
                ExtractedPaystub(
...
def _empty_mortgage_agent() -> _StubAgent:
    return _StubAgent(
        ExtractedMortgage(
...
async def test_csv_statement_parses_rows(session_factory, csv_document):
    async with session_factory() as session:
        count = await parse_service.parse_document(
            session,
            csv_document.document_id,
            statement_agent=_empty_statement_agent(),
            paystub_agent=_empty_paystub_agent(),
            mortgage_agent=_empty_mortgage_agent(),
        )
    assert count == 3
...
async def test_paystub_maps_known_labels_and_flags_unknown(
    ...
    monkeypatch.setattr(
        parse_service, "extract_pdf_text", lambda path: "stub paystub text"
    )
    paystub_agent = _StubAgent(
        ExtractedPaystubs(
            paystubs=[
                ExtractedPaystub(
                    pay_date=date(2026, 1, 15),
...
async def test_paystub_with_two_periods_extracts_all_lines(
    ...
    monkeypatch.setattr(
        parse_service, "extract_pdf_text", lambda path: "stub two-period paystub text"
    )
    paystub_agent = _StubAgent(
        ExtractedPaystubs(
            paystubs=[
...
async def client(session_factory):
    ...
    app.dependency_overrides[get_statement_extractor] = _empty_statement_agent
    app.dependency_overrides[get_paystub_extractor] = _empty_paystub_agent
    app.dependency_overrides[get_mortgage_extractor] = _empty_mortgage_agent
...
async def test_opening_balances_creates_balanced_journal_entry(
    ...
    async with session_factory() as session:
        line_count = await parse_service.parse_document(
            session,
            opening_balances_document.document_id,
            statement_agent=_empty_statement_agent(),
            paystub_agent=_empty_paystub_agent(),
            mortgage_agent=_empty_mortgage_agent(),
        )
